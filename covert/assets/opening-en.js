...
(function () {
  ...
  /* 여섯 번 넘긴다. 넘김 사이사이에 «직접 쓰는» 구간이 넷 들어간다 —
       ...
       6·7   경복궁 기록면 + 인장 스티커
       36·37 임무 기록 두 장 */
  /* ⚠️ 마지막 넘김은 .970 에 «끝나야» 한다. 1.000 까지 끌면 뒤표지를 보여 주는
     시간이 0 이 되고, 마지막 자막도 다 뜨자마자 핀이 풀린다 (대표 신고 · 2026-09-15) */
  var SPANS = [[.010, .040], [.280, .040], [.385, .040],
               [.700, .040], [.900, .040], [.940, .030]];
  ...
  var WRITE = [
    ...
    ['id02',   .062, .026],
    ['fname',  .124, .013], ['fid',   .139, .015], ['fcode', .156, .012],
    ['fnat',   .170, .012], ['fdob',  .184, .014], ['fsex',  .200, .008],
    ['fmbti',  .210, .010], ['fcolor', .222, .012],

    /* 4·5 서약 서명 */
    ['p04sign', .335, .030],

    /* 6·7 경복궁 기록면 */
    ['hd1', .448, .012], ['hd2', .463, .012],
    ['hwx', .480, .012],
    ['hy1', .497, .019], ['hy2', .519, .019], ['hy3', .541, .017], ['hy4', .561, .017],
    ['hcx', .582, .010],
    ['hm',  .596, .015],
    ['hc',  .615, .011],
    ['hk1', .630, .008], ['hk2', .639, .008], ['hk3', .648, .008],
    ['wg',  .660, .014],
    ['wh',  .676, .012],

    /* 36·37 임무 기록. 왼쪽(물건)을 먼저 채우고 오른쪽(마음)으로 넘어간다.
       ⚠️ .891 에 끝나야 한다 — 다음 장이 .900 에 넘어가기 시작한다 */
    ['mf1', .745, .009], ['mf2', .753, .009], ['mf3', .761, .008],
    ['mtick', .768, .006],
    ['mi1', .774, .008], ['mp1', .781, .007],
    ['mi2', .787, .008], ['mp2', .794, .007],
    ['msite', .800, .008],
    ['mn1', .807, .010], ['mn2', .816, .011], ['mn3', .826, .010],
    ['sp1', .836, .008], ['se1', .843, .010], ['se2', .852, .008],
    ['sm1', .860, .009], ['sm2', .868, .010], ['sm3', .877, .009],
    ['stick', .885, .006]
  ];

  /* 책을 옆으로 돌리는 구간 — 세로쓰기 칸을 채우는 동안만 90° 다 */
  var TURN_IN = [.094, .024], TURN_OUT = [.240, .024];

  /* 지면마다 붙는 설명이 «보이는 구간» — [들어옴, 나감] */
  /* [들어오기 시작, 나가기 시작, (선택) 들어오는 시간 — 기본 .025] */
  /* ⚠️ 마지막 칸은 «끝나는 값»이 9 다 — 한 번 뜨면 안 지운다.
     .945 는 뒤표지 넘김이 끝나는 .970 «전»에 다 뜨도록 당겨 둔 값이다 */
  var CAPS = [[.045, .275], [.315, .380], [.420, .695],
              [.735, .895], [.905, .938], [.945, 9, .012]];
  function renderPages(p) {
    ...
    /* 펼침 정도 — 표지가 열리면 1, 마지막 장이 넘어가면 다시 0.
       책 그림자가 이 값으로 넓어졌다 좁아진다 */
    var t0 = seg(p, SPANS[0][0], SPANS[0][1]);
    var tL = seg(p, SPANS[5][0], SPANS[5][1]);
    stage.style.setProperty('--open', (t0 * (1 - tL)).toFixed(4));
    ...
    for (var ci = 0; ci < CAPS.length; ci++) {
      ...
      /* ⚠️ 들어오는 시간은 «칸마다» 달라도 된다 (세 번째 값, 기본 .025).
         마지막 자막은 타임라인이 1.0 에서 끝나 버려 기본값으로는 영영 다 못 뜬다 —
         .990 에서 시작해 .025 에 걸쳐 들어오면 1.0 에서 40% 다.
         2026-09-14 에 실측으로 확인했다. 손으로 맞춘 다른 값은 하나도 안 건드리고
         그 칸에만 짧은 시간을 준다 */
      pagecopy.style.setProperty('--cap' + (ci + 1),
        (seg(p, CAPS[ci][0], CAPS[ci][2] || .025) * (1 - seg(p, CAPS[ci][1], .025))).toFixed(4));
    }
